...
MYSQL_INIT='/etc/init.d/mysql restart'
PHP_FPM_INI='/etc/php/7.0/fpm/'
PHP_CLI_INI='/etc/php/7.0/cli/'
MYSQL_MYCNF='/etc/mysql/'
...
# Install and/or upgrade Nginx to mainline
      echo -e  "\nDo you want to install Nginx fcgiwrap (Skip if latest Nginx is installed) (y/n)"
      read INSTALLNGINX
...
# Install Percona MySQL
      echo "Do you want to install Percona MySQL Server 5.7 (y/n)"
      read CHECKMYSQL
   if [ $CHECKMYSQL == "y" ]; then
...
# -------
   ...
   if [ $CHECKPHPINI == "y" ]; then
      ...
      cp -r $CURRENT_DIR phpini/php.ini $CONFIGFPM
      CONFIGCLI=$PHP_CLI_INI
      cp -r $CURRENT_DIR phpini/php.ini $CONFIGCLI
...
# -------
     ...
     cp -r $CURRENT_DIR/nginx.conf $CONFIGCONF
...
# Create a new domain vhost
     ...
     cp -r $CURRENT_DIR/nginx.vhost.template $CONFIG
...
# Create a new php fpm pool config
     ...
     cp -r $CURRENT_DIR/conf.template $FPMCONF
...
# Create a index.php placeholder page to avoid 403 error
     ...
     cp -r $CURRENT_DIR skel/index.php $CONFIG
